...
# --- Stochastic Composer (Single-file UI + Engine + MIDI + MusicXML) ---
...
def build_scale(key_root: str, mode: str, octaves=(4,5)) -> List[int]:
    """Return MIDI pitches for the diatonic scale over the given octave range.
       key_root is like 'C','C#','D','Eb',... (see NOTE_NAMES_SHARP)"""
    root_pc = NOTE_NAME_TO_SEMITONE[key_root]
    steps = MAJOR_STEPS if mode.lower() == "major" else NAT_MINOR_STEPS

    # Build one octave of pitch classes (scale degrees) relative to C as 0
    pcs = [0]
    for step in steps[:-1]:
        pcs.append(pcs[-1] + step)

    # Now shift these degree offsets so that C maps to root_pc
    # We'll transpose later; here we keep C-based degrees.
    notes = []
    for oct_ in range(octaves[0], octaves[1]+1):
        base = 12 * (oct_ + 1)  # MIDI C4 = 60
        for deg in pcs:
...
def transpose_to_key(pitches: List[int], key_root: str) -> List[int]:
    """Shift a C-based pitch list so that 60 (C4) maps to key_root in octave 4."""
    root_pc = NOTE_NAME_TO_SEMITONE[key_root]
    delta = root_pc  # C=0
...
STYLE_LIBRARY: Dict[str, Dict] = {
    "Classical": {
        ...
        "rhythms": [480, 240, 240, 120, 120, 360],  # q, e, e, 16th, 16th, dotted-e
        "interval_weights": {0:3, 2:8, -2:8, 4:2, -4:2, 5:1, -5:1, 7:1, -7:1},
        "chord_tone_bias": 0.65,
    },
    "Jazz": {
        "tempo": (120, 180),
        "program": 4,  # Electric Piano 1
        "rhythms": [240, 240, 240, 120, 360, 480],
        "interval_weights": {0:2, 1:6, -1:6, 2:6, -2:6, 3:2, -3:2, 4:3, -4:3},
        "chord_tone_bias": 0.5,
    },
    "Waltz": {
        "tempo": (84, 108),
        "program": 0,
        "rhythms": [320, 160, 160, 160, 480],  # heuristic 3/4 feel
        "interval_weights": {0:2, 2:7, -2:7, 4:3, -4:3, 5:1, -5:1},
        "chord_tone_bias": 0.6,
        "meter": (3,4),
    },
    "Blues": {
        ...
        "rhythms": [360, 120, 240, 240, 480],
        "interval_weights": {0:2, 1:3,-1:3, 2:6,-2:6, 3:4,-3:2, 4:2,-4:2, 6:1,-6:1},
        "chord_tone_bias": 0.55,
    },
    "Rock": {
        "tempo": (100, 140),
        "program": 30,  # Distortion Guitar
        "rhythms": [240, 240, 120, 120, 480],
        "interval_weights": {0:2, 2:7, -2:7, 4:3, -4:3, 7:2, -7:2},
        "chord_tone_bias": 0.45,
    },
    "Pop": {
        "tempo": (96, 128),
        "program": 0,
        "rhythms": [240, 240, 240, 120, 480],
        "interval_weights": {0:3, 2:8, -2:8, 4:2, -4:2, 5:2, -5:2},
        "chord_tone_bias": 0.5,
    },
    "HipHop": {
        "tempo": (78, 96),
        "program": 1,  # Bright Acoustic Piano
        "rhythms": [240, 120, 120, 480, 360],
        "interval_weights": {0:4, 1:6, -1:6, 2:4, -2:4, 3:2, -3:2},
        "chord_tone_bias": 0.4,
    },
    "Techno": {
        "tempo": (120, 140),
        "program": 81,  # Lead 2 (sawtooth)
        "rhythms": [120, 120, 120, 120, 240, 480],
        "interval_weights": {0:2, 1:5, -1:5, 2:5, -2:5, 3:2, -3:2},
        "chord_tone_bias": 0.35,
    },
    "Ambient": {
        "tempo": (60, 84),
        "program": 89,  # Pad 2 (warm)
        "rhythms": [480, 720, 960],
        "interval_weights": {0:5, 2:4, -2:4, 5:2, -5:2, 7:1, -7:1},
        "chord_tone_bias": 0.7,
    },
    "FolkGuitar": {
        "tempo": (92, 120),
        "program": 24,  # Nylon Guitar
        "rhythms": [240, 240, 360, 120, 480],
        "interval_weights": {0:2, 2:7, -2:7, 3:2, -3:2, 4:2, -4:2, 5:2, -5:2},
        "chord_tone_bias": 0.55,
    },
    "MiddleEast": {
        "tempo": (90, 120),
        "program": 104,  # Sitar (proxy)
        "rhythms": [240, 240, 120, 120, 360],
        "interval_weights": {0:2, 1:6, -1:6, 2:5, -2:5, 3:2, -3:2, 6:1, -6:1},
        "chord_tone_bias": 0.5,
    },
    "Carnatic": {
        "tempo": (84, 112),
        "program": 104,  # Sitar (proxy)
        "rhythms": [120, 120, 120, 240, 360],
        "interval_weights": {0:2, 1:5,-1:5, 2:6,-2:6, 3:3,-3:3, 4:2,-4:2},
        "chord_tone_bias": 0.6,
    },
    "Latin": {
        "tempo": (96, 132),
        "program": 0,
        "rhythms": [240, 120, 120, 360, 480],
        "interval_weights": {0:2, 2:6,-2:6, 3:3,-3:3, 4:2,-4:2, 5:2,-5:2},
        "chord_tone_bias": 0.5,
    },
    "Reggae": {
        "tempo": (70, 88),
        "program": 2,
        "rhythms": [240, 240, 480, 360],
        "interval_weights": {0:3, 2:6,-2:6, 4:3,-4:3, 7:2,-7:2},
        "chord_tone_bias": 0.5,
    },
    "LoFi": {
        "tempo": (60, 84),
        "program": 5,  # Electric Piano 2
        "rhythms": [240, 240, 120, 360, 480],
        "interval_weights": {0:4, 1:5,-1:5, 2:5,-2:5, 3:2,-3:2},
        "chord_tone_bias": 0.55,
    },
    "Baroque": {
        "tempo": (88, 120),
        "program": 7,  # Harpsichord
        "rhythms": [240, 240, 120, 120, 360, 480],
        "interval_weights": {0:2, 2:8,-2:8, 4:4,-4:4, 5:3,-5:3},
        "chord_tone_bias": 0.7,
    },
    "Cinematic": {
        "tempo": (72, 110),
        "program": 48,  # String Ensemble 1
        "rhythms": [480, 720, 960, 240],
        "interval_weights": {0:4, 2:5,-2:5, 5:3,-5:3, 7:2,-7:2},
        "chord_tone_bias": 0.65,
    },
    "Chiptune": {
        "tempo": (120, 160),
        "program": 80,  # Lead 1 (square)
        "rhythms": [120, 120, 120, 240, 480],
        "interval_weights": {0:2, 2:6,-2:6, 4:3,-4:3, 7:2,-7:2},
        "chord_tone_bias": 0.45,
    },
}
...
class StochasticComposer:
    ...
    def _pick_next_pitch(self, current: int, pool: List[int], style: Dict, chord_bias: float, key_root:str, mode:str) -> int:
        ...
        candidate = clamp(candidate, 48, 84)
        return candidate
    ...
    def generate(self,
                 ...
                 seed: Optional[int] = None) -> Tuple[List[NoteEvent], Dict]:
        if seed is not None:
            random.seed(seed)

        style = STYLE_LIBRARY[style_name]
        bpm_final = self._choose_tempo(style_name, bpm)
        meter = style.get("meter", (4,4))  # default 4/4
        ...
        for _ in range(bars):
            ...
            for dur in rhythm_seq:
                ...
                notes.append(NoteEvent(start=t, duration=dur, pitch=pitch, velocity=vel))
                t += dur
                current_pitch = pitch
...
def write_midi(filename: str, notes: List[NoteEvent], meta: Dict):
    ...
    # Build a unified list of events: (time, status, pitch, velocity, channel)
    events_list = []
    for n in notes:
    ...
    # Sort by time, with note-offs before note-ons at the same time to avoid zero-length ties
    ...
    # Program change ch0
    events += vlf_write(0) + bytes([0xC0, clamp(program,0,127)])

    # Emit events with correct delta times
    last_time = 0
...
def pitch_to_xml(pitch: int) -> Tuple[str, int, int]:
    ...
    alters = [0,1,0,-1,0,0,1,0,-1,0,-1,0]  # C,C#,D,Eb,E,F,F#,G,Ab,A,Bb,B
    step = steps[pitch % 12]
...
def write_musicxml(filename: str, notes: List[NoteEvent], meta: Dict):
    ...
    # Build a linear timeline of notes (with rests)
    events = []
    cursor = 0
...
# =========================
# Tkinter UI (runs only when executed as a script)
# =========================
...
def run_ui():
    ...
    class App(tk.Tk):
        def __init__(self):
            ...
            btn_frm.grid(row=row, column=0, columnspan=6, pady=(12,6), sticky='w')
        ...
        def _parse_seed(self):
            try:
                return int(self.seed_var.get()) if self.seed_var.get().strip() else None
            except:
                return None

        def _parse_tempo(self):
            try:
                return int(self.tempo_var.get()) if self.tempo_var.get().strip() else None
            except:
                return None
        ...
        def on_generate(self):
            ...
            self.log.insert("end", f"Generated {len(notes)} notes. Style={style}, Key={key} {scale}, PitchMode={pitch_mode}\n")
            self.log.insert("end", f"BPM={meta['bpm']}, Program={meta['program']}, Meter={meta['meter']}\n")
            self.log.insert("end", "Preview (first 10 notes):\n")
            for n in notes[:10]:
                self.log.insert("end", f"t={n.start} dur={n.duration} pitch={n.pitch} vel={n.velocity}\n")

        def on_export_midi(self):
            if not self.last_notes:
                messagebox.showinfo("Info","Please Generate first.")
                return
            fn = filedialog.asksaveasfilename(defaultextension=".mid", filetypes=[("MIDI","*.mid")])
            if not fn:
                return
            path = write_midi(fn, self.last_notes, self.last_meta)
            messagebox.showinfo("Saved", f"MIDI saved to {path}")
        ...
        def on_export_xml(self):
            ...
            messagebox.showinfo("Saved", f"MusicXML saved to {path}")

    app = App()
    app.mainloop()

if __name__ == "__main__":
    # Only run the UI when launched as a script
    run_ui()
